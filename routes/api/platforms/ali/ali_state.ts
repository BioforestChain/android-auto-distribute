...
export const app_state = async () => {
  ...
  const searchParams = new URLSearchParams({ "key": appName });
  const response = await fetch(
    `https://www.wandoujia.com/search?${searchParams}&source=index`,
    {
      method: "GET",
      headers: {
        "Referer":
          `https://www.wandoujia.com/search?${searchParams}&source=detail`,
      },
    },
  );
